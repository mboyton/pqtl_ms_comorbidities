############################################
# Two-sample Mendelian randomisation (MR) workflow:
#  1) Proteome-wide cis-pQTL → MS risk MR screen (IVW + FDR)
#  2) Comorbidity-focused MR follow-up (PheWAS-style) for
#     proteins prioritised in the primary MS screen
#
# Data sources:
# - Plasma protein instruments: deCODE genetics (SOMAscan v4 pQTLs)
# - MS GWAS: OpenGWAS (ieu-b-18)
# - Comorbid GWAS: OpenGWAS (IDs defined below)
#
# Notes:
# - Exposures must be formatted as TwoSampleMR exposure data.
# - Outcome data for comorbidities are pulled via extract_outcome_data().
# - Harmonisation uses harmonise_data(action=2) to align alleles and
#   drop incompatible palindromic SNPs.
# - Primary screen uses IVW and Benjamini–Hochberg FDR correction.
############################################
